...
interface Props {
  ...
  runLabel: string;
  runOptions: string[];
  selectedRunScript: string;
  onSelectRunScript: (script: string) => void;
  onRun: () => void;
  onBuild: () => void;
  onStop: () => void;
  onFind: () => void;
...
export function Toolbar({ running, runLabel, runOptions, selectedRunScript, onSelectRunScript, onRun, onBuild, onStop, onFind, onOpenFolder, onCreateFile, onCreateFolder, isRealWorkspace }: Props) {
  return (
    <div className="flex h-8 items-center bg-[#3c3f41] px-2 text-[#bbbbbb]">
      {/* run configuration */}
      <div className="flex items-center gap-1 rounded px-1.5 py-0.5 hover:bg-[#464a4d]">
        <span className="grid h-4 w-4 place-items-center rounded-[3px] bg-[#3178c6] text-[7px] font-bold text-white">
          TS
        </span>
        <select
          value={selectedRunScript}
          onChange={(event) => onSelectRunScript(event.target.value)}
          disabled={runOptions.length === 0}
          title="Run configuration"
          className="max-w-36 bg-transparent text-[12px] text-[#dcdcdc] outline-none disabled:opacity-80"
        >
          {runOptions.length === 0 ? <option value="">No Run Config</option> : runOptions.map((script) => <option key={script} value={script}>{script}</option>)}
        </select>
      ...
      <button onClick={onBuild} title="Build Project (F9)" className="grid h-6 w-6 place-items-center rounded hover:bg-[#464a4d]">
